...
esp_err_t connect_tcp_server(void)
{
    ...
    serverInfo.sin_addr.s_addr = inet_addr("192.168.xx");//0x0100007f;   //????????????????????????
...
void app_main(void)
{
	...
	//initialize storage (na potrzeby przechowania configu wifi)
    esp_err_t ret = nvs_flash_init(); //non-volatile storage located in flash
    if (ret == ESP_ERR_NVS_NO_FREE_PAGES || ret == ESP_ERR_NVS_NEW_VERSION_FOUND) { //sprawdz czy we flash jest wolny obszar
      ESP_ERROR_CHECK(nvs_flash_erase()); //ESP_ERROR_CHECK sprawdza czy funkcja zwraca 0, jeśli nie wyrzuca abort
      ret = nvs_flash_init();
    }
    ESP_ERROR_CHECK(ret); //upewnij sie ze nie ma błędów
